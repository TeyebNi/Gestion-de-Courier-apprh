                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('circuit.router', $d) }}" method="post" class="form-inline">
                                        @csrf
                                        @if($d->remarque_maire)
                                            <select name="service_assigne" class="form-control form-control-sm mr-2" required>
                                                <option value="">Sélectionner le service</option>
                                                @foreach($orientations as $o)
                                                    <option value="{{ $o->name }}">{{ $o->name }}</option>
                                                @endforeach
                                            </select>
                                            <button type="submit" class="btn btn-warning btn-sm">Envoyer au Service</button>
                                        @else
                                            <button type="submit" class="btn btn-success btn-sm">Retourner à l'Accueil</button>
                                        @endif
                                    </form>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted">Aucune demande à orienter.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
