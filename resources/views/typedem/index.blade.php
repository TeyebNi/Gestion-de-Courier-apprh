        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                ...
                <div class="d-flex align-items-center flex-wrap">
                    @include('partials.search-box', ['route' => 'typedem.index', 'placeholder' => 'Rechercher un type de demande...'])
  ...
  <div class="modal-dialog  modal-notify modal-lg modal-right modal-success" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Ajouter Type de Demande</h5>
      ...
      <div class="modal-body">
         ...
         <div id="typedem-ajax-errors"></div>
         <form id="typedemForm" action="{{ route('typedem.store') }}" method="post">
        @csrf
      ...
      <div class="modal-footer">
        ...
        <button type="submit" id="typedemBtn" class="btn btn-success" title="Ajouter"><i class="now-ui-icons ui-1_check"></i></button>
      </div>
      </form>
    </div>
  </div>
...
<div class="modal fade" id="exampleModal-edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog  modal-notify modal-lg modal-right modal-success" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Modifier le Type de Demande</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="editTypedemForm" action="" method="post">
       @csrf
        @method('PUT')
        <div class="modal-body">
          <div class="input-group">
...
<div class="modal fade left " id="exampleModal-delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog notifi modal-lg modal-right modal-danger" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Supprimer le Type de Demande</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="deleteTypedemForm" action="" method="post">
        @csrf
        @method('DELETE')
        <div class="modal-body">
          <p>Voulez-vous vraiment supprimer ce type de demande ?</p>
        </div>
        <div class="modal-footer">
...
@if (session('success'))
<div id="typedemEditSuccessOverlay" style="position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:2000; display:flex; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:12px; padding:32px; width:90%; max-width:380px; text-align:center; box-shadow:0 10px 30px rgba(0,0,0,0.2);">
        ...
        <button id="closeTypedemEditSuccess" class="btn btn-info" style="width:100%;">OK</button>
    </div>
</div>
@endif

@if (session('error'))
<div id="typedemErrorOverlay" style="position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:2000; display:flex; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:12px; padding:32px; width:90%; max-width:380px; text-align:center; box-shadow:0 10px 30px rgba(0,0,0,0.2);">
        ...
        <button id="closeTypedemError" class="btn btn-danger" style="width:100%;">OK</button>
    </div>
</div>
@endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (closeBtnTypedemEdit) {
        closeBtnTypedemEdit.addEventListener('click', function () {
            document.getElementById('typedemEditSuccessOverlay').style.display = 'none';
        });
    }

    var closeBtnTypedemError = document.getElementById('closeTypedemError');
    if (closeBtnTypedemError) {
        closeBtnTypedemError.addEventListener('click', function () {
            document.getElementById('typedemErrorOverlay').style.display = 'none';
        });
    }
});
</script>

<!-- Modale de succès Type Demande -->
<div id="typedemSuccessOverlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:2000; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:12px; padding:32px; width:90%; max-width:380px; text-align:center; box-shadow:0 10px 30px rgba(0,0,0,0.2);">
        <div style="margin:0 auto 16px; width:64px; height:64px; border-radius:50%; border:3px solid #28a745; display:flex; align-items:center; justify-content:center;">
            <span style="color:#28a745; font-size:32px;">&#10003;</span>
        </div>
        <h5 style="margin-bottom:8px;">Succès !</h5>
        <p id="typedemSuccessDetails" style="color:#666; margin-bottom:20px;"></p>
        <button id="closeTypedemSuccess" class="btn btn-info" style="width:100%;">OK</button>
    </div>
</div>

<script>
document.getElementById('typedemForm').addEventListener('submit', function (e) {
    e.preventDefault();

    var form = e.target;
    var btn = document.getElementById('typedemBtn');
    var errorsBox = document.getElementById('typedem-ajax-errors');
    ...
    .then(function (result) {
        ...
        if (result.status === 200 && result.data.success) {
            document.getElementById('typedemSuccessDetails').textContent = result.data.message;
            document.getElementById('typedemSuccessOverlay').style.display = 'flex';
            form.reset();
        } else if (result.status === 422) {
            var errors = result.data.errors || {};
            Object.keys(errors).forEach(function (field) {
                var span = document.querySelector('[data-error-for="' + field + '"]');
                var input = document.getElementById('typedem_' + field);
...
document.getElementById('closeTypedemSuccess').addEventListener('click', function () {
    document.getElementById('typedemSuccessOverlay').style.display = 'none';
